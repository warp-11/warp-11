...
| Warp 11 | `Stream<'p>` with typed payloads from a shared `Layout` and `Union2` sum types. A stream is a *value you apply a sink to*, so chains are ordinary function application and the ready chain runs backwards through forward composition: `streamSource inPorts |> stage |> streamMap bump |> stage |> streamSink outPorts`. The boundary is a declared *group* rather than loose ports —
`streamInputPorts`/`streamOutputPorts` declare it from the same `Layout` the
payload uses, and five helpers reach it: `streamSource`/`streamSink` for a
module's own boundary, `streamDrive` where the body builds the produced stream
itself (its `ready` is the port, so there is nothing to hand back), and
`streamToInputPorts`/`streamOfOutputPorts` for the same boundary seen from the
caller, where every direction is flipped. Operators: `streamStageFor`/`streamMap`/`streamMapTo`/`streamFifo`/`streamBroadcast`/`streamBalance`/`streamMerge2`/`streamMergeTree`/`streamConflate3`/`streamProbe`; `wormhole` + `wormholeOut`/`wormholeIn` for fan shapes (flat or clustered ~√N registered tree); `rangeStream` (a scan's read schedule as a value: 0…n−1 per start pulse), `streamWindow` (the last k beats as one window — FIR taps, 1D receptive fields; framed or free-running), `streamStride` (every n-th beat, frame-aligned — a pooling layer's stride, a decimator's rate change), `lineWindow` over the window, and `bandedWorld` (a double-buffered, band-striped 2D world store that hands lockstep engines their slices with the vertical halo routed — the working set under a stencil sweep, from Life to pooling) (raster rows in, k-row stencil windows out, shaped by a `WindowSpec` — kernel height, edge columns, cell width, edge policy; halo rows come from the loader, the horizontal edge is applied by widening, and a checked frame counter re-arms it between frames) |
...
- **The whole loop, on real silicon.** Three non-trivial accelerators on a $300 KV260, each authored entirely in the DSL with Vivado only as the place-and-route backend:
  ...
  - [**Game of Life**](../hdl/Warp11.GoL/README.md) — 64×64 at 166.67 MHz, the whole grid in one cycle: **167M generations/s**, CONFLATE/N=3 triple-buffered into PS DDR for tear-free display.
