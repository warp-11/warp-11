...
################################################################
# This is a generated script based on design: mandelfs_bd
#
...
\
  ...
  # Create instance: MandelPodAxi_0, and set properties
  set block_name MandelPodAxi
  set block_cell_name MandelPodAxi_0
  if { [catch {set MandelPodAxi_0 [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2095 -severity "ERROR" "Unable to add referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   } elseif { $MandelPodAxi_0 eq "" } {
  ...
  # Create instance: irq_concat — feeds pl_ps_irq0[0] (tied to 0 below); the
  ...
  connect_bd_intf_net -intf_net axi_smc_M00_AXI [get_bd_intf_pins axi_smc/M00_AXI] [get_bd_intf_pins MandelPodAxi_0/s_axi]
  ...
  [get_bd_pins MandelPodAxi_0/s_axi_aresetn] \
  ...
  [get_bd_pins MandelPodAxi_0/s_axi_aclk] \
  ...
  # MandelPodAxi has no irq port (run-once, the driver polls done) — In0 ties
  # to a constant 0 so the concat validates; the SPI 89 route stays in place
  # for the fast-follow that adds one.
  set irq_tie [ create_bd_cell -type ip -vlnv xilinx.com:ip:xlconstant:1.1 irq_tie ]
  set_property -dict [list \
    CONFIG.CONST_VAL {0} \
    CONFIG.CONST_WIDTH {1} \
  ] $irq_tie
  connect_bd_net -net mandelfs_irq \
    [get_bd_pins irq_tie/dout] \
  ...
  assign_bd_address -offset 0xB0000000 -range 0x00008000 -target_address_space [get_bd_addr_spaces zynq_ultra_ps_e_0/Data] [get_bd_addr_segs MandelPodAxi_0/s_axi/reg0] -force
