#*****************************************************************************************
# build_gepunit_axi.tcl — create + build the GEP real-engine work-queue pool bitstream (gep_design.md §3).
#
# Reads the emitted RTL from ../build/GepUnitPool_axi.v (run
# `./gradlew :examples:gep:emitHardware` first), recreates the block design via
# gepunit_bd_bd.tcl (Zynq PS + smartconnect + the GepUnitPool_axi module reference at
...
#
# Usage:
#   cd hardware/vivado && vivado -mode batch -source build_gepunit_axi.tcl
#
# Output:
#   gepunit_axi/gepunit_axi.runs/impl_1/gepunit_bd_wrapper.bit
#*****************************************************************************************

set origin_dir [file normalize [file dirname [info script]]]
set proj_name  "gepunit_axi"
...
set src_v [file normalize "${origin_dir}/../build/GepUnitPool_axi.v"]
...
set patched_v "${proj_dir}/GepUnitPool_axi_patched.v"
...
# Recreate the block design (instantiates the GepUnitPool_axi module reference).
source [file normalize "${origin_dir}/gepunit_bd_bd.tcl"]

set top_bd [get_files "${proj_dir}/${proj_name}.srcs/sources_1/bd/gepunit_bd/gepunit_bd.bd"]
make_wrapper -fileset sources_1 -files $top_bd -top
add_files -norecurse -fileset sources_1 \
    [file normalize "${proj_dir}/${proj_name}.gen/sources_1/bd/gepunit_bd/hdl/gepunit_bd_wrapper.v"]
set_property top "gepunit_bd_wrapper" [get_filesets sources_1]
...
set bit [file normalize "${proj_dir}/${proj_name}.runs/impl_1/gepunit_bd_wrapper.bit"]
if { ![file isfile $bit] } {
