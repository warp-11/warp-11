...
################################################################
# This is a generated script based on design: audio_passthru_bd
#
...
# source audio_passthru_bd_script.tcl
...
set design_name audio_passthru_bd
...
\
  ...
  # Create instance: AudioPassthru_axi_0, and set properties
  set block_name AudioPassthru_axi
  set block_cell_name AudioPassthru_axi_0
  if { [catch {set AudioPassthru_axi_0 [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2095 -severity "ERROR" "Unable to add referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   } elseif { $AudioPassthru_axi_0 eq "" } {
  ...
  connect_bd_intf_net -intf_net axi_smc_M00_AXI [get_bd_intf_pins axi_smc/M00_AXI] [get_bd_intf_pins AudioPassthru_axi_0/s_axi]
  ...
  [get_bd_pins AudioPassthru_axi_0/s_axi_aresetn] \
  ...
  [get_bd_pins AudioPassthru_axi_0/s_axi_aclk] \
  ...
  # AudioPassthru has no interrupt (no irq port on the slave). irq_concat is left
  # with all inputs tied to 0 (xlconcat default) and still drives pl_ps_irq0
  # so the PS irq input stays connected.
  connect_bd_net -net irq_concat_dout \
    [get_bd_pins irq_concat/dout] \
    [get_bd_pins zynq_ultra_ps_e_0/pl_ps_irq0]

  # Codec pins → external BD ports (become top-level wrapper ports of the
  # same name; the XDC pins them to KV260 J2 Pmod). Top row = CS4344 DAC
  # out (mclk/lrclk/sclk/sdin + the shared ADC clocks mclk2/lrclk2/sclk2);
  # sdout is the lone input (CS5343 ADC serial data on the bottom row).
  foreach p {mclk lrclk sclk sdin mclk2 lrclk2 sclk2} {
    create_bd_port -dir O $p
    connect_bd_net [get_bd_pins AudioPassthru_axi_0/$p] [get_bd_ports $p]
  }
  create_bd_port -dir I sdout
  connect_bd_net [get_bd_pins AudioPassthru_axi_0/sdout] [get_bd_ports sdout]

  # Create address segments
  assign_bd_address -offset 0xB0000000 -range 0x00001000 -target_address_space [get_bd_addr_spaces zynq_ultra_ps_e_0/Data] [get_bd_addr_segs AudioPassthru_axi_0/s_axi/reg0] -force
