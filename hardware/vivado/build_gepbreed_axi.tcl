#*****************************************************************************************
# build_gepbreed_axi.tcl — create + build the GEP streaming breeding pool bitstream (gep_design.md §4).
#
# Reads the emitted RTL from ../build/GepBreed_axi.v (run
# `./gradlew :examples:gep:emitHardware` first), recreates the block design via
# gepbreed_bd_bd.tcl (Zynq PS + smartconnect + the GepBreed_axi module reference at
...
# gepbreedlane_ooc logs); check_timing gates the build if post-route timing
# misses.
#
# Usage:
#   cd hardware/vivado && vivado -mode batch -source build_gepbreed_axi.tcl
#
# Output:
#   gepbreed_axi/gepbreed_axi.runs/impl_1/gepbreed_bd_wrapper.bit
#*****************************************************************************************

set origin_dir [file normalize [file dirname [info script]]]
set proj_name  "gepbreed_axi"
...
set src_v [file normalize "${origin_dir}/../build/GepBreed_axi.v"]
...
set patched_v "${proj_dir}/GepBreed_axi_patched.v"
...
# Recreate the block design (instantiates the GepBreed_axi module reference).
source [file normalize "${origin_dir}/gepbreed_bd_bd.tcl"]

set top_bd [get_files "${proj_dir}/${proj_name}.srcs/sources_1/bd/gepbreed_bd/gepbreed_bd.bd"]
make_wrapper -fileset sources_1 -files $top_bd -top
add_files -norecurse -fileset sources_1 \
    [file normalize "${proj_dir}/${proj_name}.gen/sources_1/bd/gepbreed_bd/hdl/gepbreed_bd_wrapper.v"]
set_property top "gepbreed_bd_wrapper" [get_filesets sources_1]
...
set bit [file normalize "${proj_dir}/${proj_name}.runs/impl_1/gepbreed_bd_wrapper.bit"]
if { ![file isfile $bit] } {
