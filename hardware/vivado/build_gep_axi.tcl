#*****************************************************************************************
# build_gep_axi.tcl — create + build the GEP engine-pool accelerator bitstream.
#
# Reads the emitted RTL from ../build/GepPool_axi.v (run
# `./gradlew :examples:gep:emitHardware` first), recreates the block design via
# gep_bd_bd.tcl (Zynq PS + smartconnect + the GepPool_axi module reference at
# AXI base 0xB0000000), then runs synth + impl + write_bitstream.
#
# GepPool_axi is slave-only (AXI-Lite s_axi, no AXI master), so unlike the
# Mandelbrot flow there is no m_axi interface-parameter patch — Vivado infers
# the s_axi bus from the s_axi_* port names.
#
# Usage:
#   cd hardware/vivado && vivado -mode batch -source build_gep_axi.tcl
#
# Output:
#   gep_axi/gep_axi.runs/impl_1/gep_bd_wrapper.bit
#*****************************************************************************************

set origin_dir [file normalize [file dirname [info script]]]
set proj_name  "gep_axi"
...
set src_v [file normalize "${origin_dir}/../build/GepPool_axi.v"]
...
# Recreate the block design (instantiates the GepPool_axi module reference).
source [file normalize "${origin_dir}/gep_bd_bd.tcl"]

set top_bd [get_files "${proj_dir}/${proj_name}.srcs/sources_1/bd/gep_bd/gep_bd.bd"]
make_wrapper -fileset sources_1 -files $top_bd -top
add_files -norecurse -fileset sources_1 \
    [file normalize "${proj_dir}/${proj_name}.gen/sources_1/bd/gep_bd/hdl/gep_bd_wrapper.v"]
set_property top "gep_bd_wrapper" [get_filesets sources_1]
...
set bit [file normalize "${proj_dir}/${proj_name}.runs/impl_1/gep_bd_wrapper.bit"]
if { ![file isfile $bit] } {
