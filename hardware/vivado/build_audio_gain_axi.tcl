# build_audio_gain_axi.tcl
#
# From-scratch reproducible build of the GameOfLife64 KV260 bitstream.
#
# Usage:
#   cd hardware/vivado
#   vivado -mode batch -source build_audio_gain_axi.tcl
#
# Inputs (must exist before running):
#   ../build/GameOfLife64x64.v     — emitted by `./gradlew :shared:emitHardware`
#   ../axi/AudioGain_axi.v      — hand-written AXI-Lite wrapper
#   ./audio_gain_bd_bd.tcl                — BD recreation script (sibling file)
#
# Output:
#   ./audio_gain_axi/audio_gain_axi.runs/impl_1/audio_gain_bd_wrapper.bit
#
# Project + BD layout mirrors the counter_axi reference. Differences from
# counter:
#   - Project name: audio_gain_axi
#   - BD name:      audio_gain_bd
#   - DUT sources:  AudioGain_axi.v (single emitted file: wrapper + i2sMaster
#                   + i2sRx + readyValidFifo + audioGain + i2sTx + AudioGainSlave)
#   - Constraints:  audio_gain_pins.xdc (J2 Pmod codec pins)
#   - Address:      0xB0000000 / 0x1000 (set in audio_gain_bd_bd.tcl, sufficient
#                   for the 4 KB GoL register map; CELL_ROW ends at 0x4FF)
#
# After build, run `cd ../xmutil && ./package_audio_gain.sh` (TODO: clone of
# package_counter.sh) to wrap into an xmutil-loadable app.

set origin_dir [file normalize [file dirname [info script]]]
set proj_name  "audio_gain_axi"
...
set rtl_files [list \
    [file normalize "${origin_dir}/../build/AudioGain_axi.v"] \
]
foreach f $rtl_files {
    if { ![file isfile $f] } {
...
set xdc [file normalize "${origin_dir}/audio_gain_pins.xdc"]
if { ![file isfile $xdc] } {
...
source [file normalize "${origin_dir}/audio_gain_bd_bd.tcl"]

# Generate the HDL wrapper around the BD and use it as the top. Target the
# top-level audio_gain_bd.bd by path — `get_files -filter {FILE_TYPE == "Block
# Designs"}` also picks up the smartconnect's internal sub-BD, which
# make_wrapper can't wrap and aborts on.
set top_bd [get_files "${proj_dir}/${proj_name}.srcs/sources_1/bd/audio_gain_bd/audio_gain_bd.bd"]
make_wrapper -fileset sources_1 -files $top_bd -top
add_files -norecurse -fileset sources_1 \
    [file normalize "${proj_dir}/${proj_name}.gen/sources_1/bd/audio_gain_bd/hdl/audio_gain_bd_wrapper.v"]
set_property top "audio_gain_bd_wrapper" [get_filesets sources_1]
...
set bit [file normalize "${proj_dir}/${proj_name}.runs/impl_1/audio_gain_bd_wrapper.bit"]
if { ![file isfile $bit] } {
