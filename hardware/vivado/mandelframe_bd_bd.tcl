...
################################################################
# This is a generated script based on design: mandelframe_bd
#
...
# source mandelframe_bd_script.tcl
...
set design_name mandelframe_bd
...
\
    ...
    CONFIG.PSU__CRL_APB__PL0_REF_CTRL__FREQMHZ {166.666672} \
...
\
  ...
  # Create instance: MandelFrameAxi_0, and set properties
  set block_name MandelFrameAxi
  set block_cell_name MandelFrameAxi_0
  if { [catch {set MandelFrameAxi_0 [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2095 -severity "ERROR" "Unable to add referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   } elseif { $MandelFrameAxi_0 eq "" } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2096 -severity "ERROR" "Unable to referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   }
  # Note: s_axi_aclk's ASSOCIATED_BUSIF is set via X_INTERFACE_PARAMETER
  # attribute injected by build_mandelpod_axi.tcl. Trying to set it here
  # fails with "It is read-only" since module-reference interfaces are
  # frozen after inference.

  # Create instance: axi_smc, and set properties
  set axi_smc [ create_bd_cell -type ip -vlnv xilinx.com:ip:smartconnect:1.0 axi_smc ]
  set_property CONFIG.NUM_SI {1} $axi_smc

  # Create instance: axi_smc_hp — smartconnect from MandelFrameAxi master
  # to PS HP0_FPD slave. Our master is already 128-bit, matching the HP
  # port's native width, so no data-width conversion happens here —
  # smartconnect is the protocol/address adapter into the HP aperture.
  ...
  # Create instance: irq_concat — collects the GoL irq into pl_ps_irq0[0]; the
  # remaining 7 inputs auto-tie to 0 (xlconcat default). NUM_PORTS=8 matches
  # the pl_ps_irq0 bus width on zynq_ultra_ps_e so Vivado doesn't need a slicer.
  ...
  connect_bd_intf_net -intf_net axi_smc_M00_AXI [get_bd_intf_pins axi_smc/M00_AXI] [get_bd_intf_pins MandelFrameAxi_0/s_axi]
  ...
  # MandelFrameAxi master → smartconnect → PS HP0_FPD slave (writes fb to PS DDR)
  connect_bd_intf_net -intf_net mandelpod_m_axi [get_bd_intf_pins MandelFrameAxi_0/m_axi] [get_bd_intf_pins axi_smc_hp/S00_AXI]
  connect_bd_intf_net -intf_net axi_smc_hp_M00_AXI [get_bd_intf_pins axi_smc_hp/M00_AXI] [get_bd_intf_pins zynq_ultra_ps_e_0/S_AXI_HP0_FPD]

  # Create port connections
  connect_bd_net -net rst_ps8_0_99M_peripheral_aresetn  [get_bd_pins rst_ps8_0_99M/peripheral_aresetn] \
  [get_bd_pins MandelFrameAxi_0/s_axi_aresetn] \
  ...
  [get_bd_pins MandelFrameAxi_0/s_axi_aclk] \
  ...
  # GoL IRQ → xlconcat[0] → pl_ps_irq0 (GIC SPI 89/0x59, level-high in DT overlay).
  connect_bd_net -net mandelpod_irq \
    [get_bd_pins MandelFrameAxi_0/irq] \
    [get_bd_pins irq_concat/In0]
  connect_bd_net -net irq_concat_dout \
    [get_bd_pins irq_concat/dout] \
    [get_bd_pins zynq_ultra_ps_e_0/pl_ps_irq0]

  # Create address segments
  assign_bd_address -offset 0xB0000000 -range 0x00001000 -target_address_space [get_bd_addr_spaces zynq_ultra_ps_e_0/Data] [get_bd_addr_segs MandelFrameAxi_0/s_axi/reg0] -force
  # Mandelbrot AXI master reaches PS DDR (low 2 GB) via the HP0_FPD slave port.
  assign_bd_address -offset 0x00000000 -range 0x80000000 -target_address_space [get_bd_addr_spaces MandelFrameAxi_0/m_axi] [get_bd_addr_segs zynq_ultra_ps_e_0/SAXIGP2/HP0_DDR_LOW] -force
