...
# are name substitutions (gol_axi → mandel_axi, gol_bd → mandel_bd,
# GameOfLife64_axi → Mandelbrot_axi) and the address range (2 MB instead
# of 4 KB, because the framebuffer at +1 MB pushes the total above the
# default 64 KB slot).
#
# Usage:
#   cd hardware/vivado
#   vivado -mode batch -source build_mandel_axi.tcl
#
# Inputs (must exist before running):
#   ../build/Mandelbrot_axi.v      — emitted by `./gradlew :shared:emitHardware`
#                                    (single file containing the wrapper
#                                    plus all transitively-referenced
#                                    submodules — engines, slave, etc.)
#   ./mandel_bd_bd.tcl             — BD recreation script (sibling file)
#
# Output:
#   ./mandel_axi/mandel_axi.runs/impl_1/mandel_bd_wrapper.bit
#
# After build, run `cd ../xmutil && ./package_mandel.sh` to wrap the
# bitstream + DT overlay into an xmutil-loadable app, then deploy with
# `./gradlew :driver:deployBitstreamToBoard -Pkv260.app=mandel`.

set origin_dir [file normalize [file dirname [info script]]]
set proj_name  "mandel_axi"
...
set rtl_files [list \
    [file normalize "${origin_dir}/../build/Mandelbrot_axi.v"] \
]
foreach f $rtl_files {
    if { ![file isfile $f] } {
...
set src_v   [file normalize "${origin_dir}/../build/Mandelbrot_axi.v"]
set patched_v "${proj_dir}/Mandelbrot_axi_patched.v"
file mkdir $proj_dir
...
source [file normalize "${origin_dir}/mandel_bd_bd.tcl"]

set top_bd [get_files "${proj_dir}/${proj_name}.srcs/sources_1/bd/mandel_bd/mandel_bd.bd"]
make_wrapper -fileset sources_1 -files $top_bd -top
add_files -norecurse -fileset sources_1 \
    [file normalize "${proj_dir}/${proj_name}.gen/sources_1/bd/mandel_bd/hdl/mandel_bd_wrapper.v"]
set_property top "mandel_bd_wrapper" [get_filesets sources_1]
...
set bit [file normalize "${proj_dir}/${proj_name}.runs/impl_1/mandel_bd_wrapper.bit"]
if { ![file isfile $bit] } {
