...
# Usage:
#   cd hardware/vivado
#   vivado -mode batch -source build_gol_axi.tcl
#
# Inputs (must exist before running):
#   ../build/GameOfLife64x64.v     — emitted by `./gradlew :shared:emitHardware`
#   ../axi/GameOfLife64_axi.v      — hand-written AXI-Lite wrapper
#   ./gol_bd_bd.tcl                — BD recreation script (sibling file)
#
# Output:
#   ./gol_axi/gol_axi.runs/impl_1/gol_bd_wrapper.bit
#
# Project + BD layout mirrors the counter_axi reference. Differences from
# counter:
#   - Project name: gol_axi
#   - BD name:      gol_bd
#   - DUT sources:  GameOfLife64x64.v + GameOfLife64_axi.v
#   - Address:      0xB0000000 / 0x1000 (set in gol_bd_bd.tcl, sufficient
#                   for the 4 KB GoL register map; CELL_ROW ends at 0x4FF)
#
# After build, run `cd ../xmutil && ./package_gol.sh` (TODO: clone of
# package_counter.sh) to wrap into an xmutil-loadable app.

set origin_dir [file normalize [file dirname [info script]]]
set proj_name  "gol_axi"
...
set rtl_files [list \
    [file normalize "${origin_dir}/../build/GameOfLife64_axi.v"] \
]
foreach f $rtl_files {
    if { ![file isfile $f] } {
...
source [file normalize "${origin_dir}/gol_bd_bd.tcl"]

# Generate the HDL wrapper around the BD and use it as the top. Target the
# top-level gol_bd.bd by path — `get_files -filter {FILE_TYPE == "Block
# Designs"}` also picks up the smartconnect's internal sub-BD, which
# make_wrapper can't wrap and aborts on.
set top_bd [get_files "${proj_dir}/${proj_name}.srcs/sources_1/bd/gol_bd/gol_bd.bd"]
make_wrapper -fileset sources_1 -files $top_bd -top
add_files -norecurse -fileset sources_1 \
    [file normalize "${proj_dir}/${proj_name}.gen/sources_1/bd/gol_bd/hdl/gol_bd_wrapper.v"]
set_property top "gol_bd_wrapper" [get_filesets sources_1]
...
set bit [file normalize "${proj_dir}/${proj_name}.runs/impl_1/gol_bd_wrapper.bit"]
if { ![file isfile $bit] } {
