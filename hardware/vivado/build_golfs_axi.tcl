# build_golfs_axi.tcl
#
# From-scratch reproducible build of the Game of Life accelerator KV260
# bitstream. Mirrors build_gol_axi.tcl — the only structural differences
# are name substitutions (gol_axi → golfs_axi, gol_bd → golfs_bd,
# GameOfLife64_axi → GolAxi) and the address range (2 MB instead
# of 4 KB, because the framebuffer at +1 MB pushes the total above the
# default 64 KB slot).
#
# Usage:
#   cd hardware/vivado
#   vivado -mode batch -source build_golfs_axi.tcl
#
# Inputs (must exist before running):
#   ../build/GolAxi.v      — emitted by `dotnet run -- hardware ../..`
#                                    (single file containing the wrapper
#                                    plus all transitively-referenced
#                                    submodules — engines, slave, etc.)
#   ./golfs_bd_bd.tcl             — BD recreation script (sibling file)
#
# Output:
#   ./golfs_axi/golfs_axi.runs/impl_1/golfs_bd_wrapper.bit
#
...
# `./gradlew :driver:deployBitstreamToBoard -Pkv260.app=golfs`.

set origin_dir [file normalize [file dirname [info script]]]
set proj_name  "golfs_axi"
...
set rtl_files [list \
    [file normalize "${origin_dir}/../build/GolAxi.v"] \
]
foreach f $rtl_files {
    if { ![file isfile $f] } {
...
set src_v   [file normalize "${origin_dir}/../build/GolAxi.v"]
set patched_v "${proj_dir}/GolAxi_patched.v"
file mkdir $proj_dir
...
source [file normalize "${origin_dir}/golfs_bd_bd.tcl"]

set top_bd [get_files "${proj_dir}/${proj_name}.srcs/sources_1/bd/golfs_bd/golfs_bd.bd"]
make_wrapper -fileset sources_1 -files $top_bd -top
add_files -norecurse -fileset sources_1 \
    [file normalize "${proj_dir}/${proj_name}.gen/sources_1/bd/golfs_bd/hdl/golfs_bd_wrapper.v"]
set_property top "golfs_bd_wrapper" [get_filesets sources_1]
...
set bit [file normalize "${proj_dir}/${proj_name}.runs/impl_1/golfs_bd_wrapper.bit"]
if { ![file isfile $bit] } {
