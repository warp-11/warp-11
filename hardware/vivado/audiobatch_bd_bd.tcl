################################################################
#
# This is a generated script based on design: audiobatch_bd
#
...
# source audiobatch_bd_script.tcl
...
\
  ...
  # Create instance: AudioBatchAxi_0, and set properties
  set block_name AudioBatchAxi
  set block_cell_name AudioBatchAxi_0
  if { [catch {set AudioBatchAxi_0 [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2095 -severity "ERROR" "Unable to add referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   } elseif { $AudioBatchAxi_0 eq "" } {
  ...
  # Create instance: axi_smc_hp — smartconnect from the AudioBatchAxi DMA
  # master (unit-queue reads + result-ring writes) to PS HPC0_FPD (coherent) slave. Auto-
  # converts the 32-bit reads / 128-bit writes to the port's native width.
  ...
  connect_bd_intf_net -intf_net axi_smc_M00_AXI [get_bd_intf_pins axi_smc/M00_AXI] [get_bd_intf_pins AudioBatchAxi_0/s_axi]
  ...
  # AudioBatchAxi DMA master → smartconnect → PS HPC0_FPD. NOTE: the F# AXI
  # masters drive AxCACHE=0 (Stdlib.fs), so these transactions do NOT snoop the
  # APU caches whatever port they land on. The app's dts therefore does not
  # claim `dma-coherent`, and the driver does explicit sync_for_device before a
  # run and sync_for_cpu after it.
  connect_bd_intf_net -intf_net audiobatch_m_axi [get_bd_intf_pins AudioBatchAxi_0/m_axi] [get_bd_intf_pins axi_smc_hp/S00_AXI]
  connect_bd_intf_net -intf_net axi_smc_hp_M00_AXI [get_bd_intf_pins axi_smc_hp/M00_AXI] [get_bd_intf_pins zynq_ultra_ps_e_0/S_AXI_HPC0_FPD]

  # Create port connections
  connect_bd_net -net rst_ps8_0_99M_peripheral_aresetn  [get_bd_pins rst_ps8_0_99M/peripheral_aresetn] \
  [get_bd_pins AudioBatchAxi_0/s_axi_aresetn] \
  ...
  [get_bd_pins AudioBatchAxi_0/s_axi_aclk] \
  ...
  assign_bd_address -offset 0xB0000000 -range 0x00001000 -target_address_space [get_bd_addr_spaces zynq_ultra_ps_e_0/Data] [get_bd_addr_segs AudioBatchAxi_0/s_axi/reg0] -force
  # AudioBatchAxi DMA master reaches PS DDR (low 2 GB) via the coherent HPC0_FPD slave port.
  assign_bd_address -offset 0x00000000 -range 0x80000000 -target_address_space [get_bd_addr_spaces AudioBatchAxi_0/m_axi] [get_bd_addr_segs zynq_ultra_ps_e_0/SAXIGP0/HPC0_DDR_LOW] -force
