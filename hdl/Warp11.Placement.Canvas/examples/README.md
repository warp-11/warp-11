...
| `mandelbrot.json` | the Mandelbrot frame: a count in, `coords` minting each chunk's view, `mandelChunk` × 4 (the lane pod at one chunk wide — 128 pixels, drained as eight beats for one — farmed by *copies*), sixteen pixels a beat out. Open it with `frame:1400x800`, type the view into the control boxes as Q4.28 bits — −2.0 … 1.0 across and 1.0 … −1.0 down is `cxOrigin` 3758096384, `cyOrigin` 268435456, `dx` 575219, `dy` 4294296207 — Run, **Save heard** writes the PGM. Its mapping is the KV260's **counted** path: the host writes the count and the view, reads the frame from DDR |
