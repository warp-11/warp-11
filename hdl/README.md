...
**New here? Start with the [tutorial](Warp11.Tutorial/doc/counter.md)** — 35
...
dotnet run --project Warp11.Tutorial.App                                       # the step-through debugger, on the teaching set
dotnet run --project Warp11.Designs.App                                        # ...on the oracle catalog, every design in it
dotnet run --project Warp11.Gep/Warp11.Gep.fsproj -- debug operator-engine     # ...on a design no catalog carries
...
Either head opens on the first design in its catalog, or on one named:
`Warp11.Designs.App -- "Filling memory (256 words)"`. That head matches the
label case-insensitively and prints the catalog when nothing answers to it,
rather than quietly opening something else. Pick signals to watch from the
filter list,
...
window on the *same* session. The two catalogs are the teaching set and the
oracle inventory; a project whose designs are in neither opens a debugger on
its own instead: `-- debug operator-engine` in `Warp11.Gep`, `-- debug
coalescer` in `Warp11.Mandelbrot`. A state machine built with `machine` shows its state names
...
| `Builder` | `Dsl.fs` | the mutable builder everything else runs on. `Instance` returns the module's port bundle over the instance's staging wires; the one-argument overload derives an instance name from the child module. **Every port's direction is recorded at declaration and every staging net's flip at `Instance`** — a child input is a net the parent must drive, a child output one it reads — so `==>` refuses a body driving its own input and a caller driving a child's output, at the connect, with the module and port named; finalize refuses a child input nobody drove. The same mechanism as Chisel's, whose `Flipped` is also elaboration-checked data, not a static type. |
| `defModule` | `Dsl.fs` | **the module former: a port bundle and one body, nothing else.** The `Ports -> 'io` factory is re-run per instance — warp11-Kotlin's re-runnable-factory trick ported directly — so `.New`/`.NewNamed` hand a call site the same typed bundle the definition saw. A richer call shape is an ordinary wrapper function beside the module (2026-09-05: the io/apply/body triple retired; nothing needed the third piece). **The boundary seals when the factory returns**: a port declared from body depth — an ambient `input`, a library call, a stashed factory — is an elaboration error, so the io factory is the whole interface and the bundle is the receipt. Bundle records say their directions with the `Input`/`Output` aliases (both `= Expr`; the enforcement is the builder's, above). |
...
| **the ambient layer** | `Dsl.fs` | `elaborating` (a thread-local stack of builders), `design`, and free `input`/`output`/`wire`/`reg`/`==>`. This is what makes a module body read as ordinary F# — dynamic scoping, the same conclusion Rust reached with `thread_local!` and Kotlin with the receiver lambda. Disciplined on purpose: only the three module formers push it, for exactly the extent of a body, and two threads elaborating independently cannot see each other's builders. |
...
| `emitDesign` (+ internal `emitVerilog` / `emitAt`) | `Verilog.fs` | children-first multi-module emission. **`emitDesign` is the whole public surface here** — it runs the three checks above and emits every module in the design, deduplicated by name. `emitVerilog` is the per-module worker it maps over: one module, no checks, so calling it on a design with instances silently drops the children. It is `internal` as of 0.1.0-alpha.3 *because* it was public: the guides taught it by mistake, since its name reads like the entry point when `emitDesign` is. The only caller that wants the worker directly is `writeDiff`, which collapses *several* designs into one `modules.v` and so has to dedupe across the whole set — and it still runs `emitDesign` first for the checks. `emitAt` zero-extends every operand to its result width so a sub-expression's width never depends on its context — see below. Registers reset to their declared init under `rst`; `clk`/`rst` thread transitively (`needsClk`), so a reg-free parent holding a stateful child still gets and forwards the pair. |
| designs | `Warp11.Designs/Designs.fs` + `Warp11.Designs/MemoryDesigns.fs` + `Warp11.Mandelbrot/Pod.fs` | `counterMutable`, `add3`, then `dot2` / `dot2Auto` / `dot2Ambient` / `dot2Inline`, the stateful pair `pipelinedDot` / `gatedCounter`, `streamPipe` (single-field stream), `coordPipe` (two-field payload — Warp 11's pixel-beat rule in miniature), the `If` pair `onCounter` (nested, reg-hold) / `onPriority` (defaulted wire, last-connect priority), the generator pair `loopPipeline` (a fold building 4 instances) / `treeSum` (recursive inline adder tree), everything whose subject is *storage* in `MemoryDesigns.fs` — `ramTest` (write under On, sync + async reads, read-first), `dualRead` (two ports, two addresses: writes fold and reads do not), `priorityWrite` (three sites, one write port, last one wins), `maskedWritePriority` (complementary masks that do *not* both land), the ROM pair `romLookup` / `blockRomLookup`, the FIFO pair `bufferedStream` / `deepBufferedStream`, the five AXI masters, and `threeStorages` (all three storages in one design: LUTs answer now, a block next cycle, DDR eventually — the "before" picture for `notes/DEVICES.md`'s UC2) — the union pair `cmdProcessor` (a union command stream driving a mem — most of the surface in one design) / `unionRoundTrip` (inject, mux, unslice), `forkJoin` (replicate → two branches → round-robin merge), and the signed pair `signedOps` (every signed op at its own port; random stimulus sets the sign bit half the time, so the 0x80/0xFF boundary patterns reach the oracle without being enumerated) / `escapeStep` (Mandelbrot's inner step in Q4.4 — coordinates declared `SInt 8`, Q renormalization as a plain slice of the product wire, escape as an *unsigned* compare because squares are non-negative, now said with an `asUInt` rather than left to a comment), and the Fixed pair `escapeStepFixed` (the same step on the typed layer — same wires, same order, byte-identical arithmetic; the escape compare is the one divergence, `fLt` being signed where the hand-written design chose the unsigned trick) / `escapeStep28` (the step at Mandelbrot's real Q4.28, whose Q8.56 products sit exactly on the narrow Sim's 64-bit ceiling — the boundary the mini pod needs, differentially exercised before the pod exists), and **`mandelLane` / `mandelPod`** — the acceptance artifact, § The pod below. `nameCollision`, `danglingStream`, `onBadWire`, `doubleAssign`, `declCollision` and `widthViolation` exist to make the checks fire. |
